...
bool checkDfsCycle(int node , int parent ,unordered_map<int,bool> visited , unordered_map<int,list<int>> adj)
{
    visited[node] = true;
    ...
    for(auto neighbour : adj[node])
    {
        ...
        {
            //                                present node,parent
            bool cyclePresent = checkDfsCycle(neighbour,node,visited,adj);
            if(cyclePresent)
            {
                return true;
            }
        ...
        // if visited and parent then neglect
    }
    return false;
}
...
string cycleDetection (vector<vector<int>>& edges, int n, int m)
{
    ...
    for(int i=0 ; i<n ; i++)
    {
        if(!visited[i])
        {
            ...
            bool ans = checkDfsCycle(i,-1,visited,adj);
            if(ans == 1)
            {
                return "Yes";
            }
